...
import { toggleCircuitsProgress } from "@/app/students/[id]/actions";

interface CircuitsProject {
	id: string;
	name: string;
	order: number;
	completed: boolean;
...
interface Props {
	studentId: string;
	projects: CircuitsProject[];
}

export default function CircuitsProgressCard({ studentId, projects }: Props) {
	...
	function handleToggle(projectId: string) {
		...
		startTransition(async () => {
			const newProgressId = await toggleCircuitsProgress(studentId, projectId, current.progressId);
			setOptimistic((prev) =>
